...
const TaskCard: React.FC<TaskCardProps> = ({ task, onUpdateDuration, onDone, onDelete }) => {
  return (
    <div className="fade-in-standard group relative mx-6 mb-2 landscape:mx-4 px-8 py-3 landscape:py-2.5 rounded-[1.8rem] card-themed overflow-hidden hover:py-5 landscape:hover:py-4 transition-all duration-300 ease-out">
      ...
      <div className="relative flex justify-between items-center z-10">
        ...
        <div className="flex items-center gap-2">
            <span className="drag-handle p-2 cursor-grab active:cursor-grabbing opacity-10 hover:opacity-100 transition-opacity landscape:hidden sm:block">
            ...
            <button 
                onClick={() => onDone(task)}
                className="h-10 w-10 landscape:h-9 landscape:w-9 flex items-center justify-center rounded-full bg-[var(--text-main)] text-[var(--bg-page)] hover:scale-110 active:scale-95 transition-all shadow-lg"
            >
      ...
      <div className="max-h-0 group-hover:max-h-20 overflow-hidden opacity-0 group-hover:opacity-100 transition-all duration-300">
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-[var(--text-main)]/10">
            <div className="flex gap-4">
              <button 
                onClick={() => onUpdateDuration(task.id, task.duration + (task.duration >= 60 ? 30 : 5))} 
              ...
              <button 
                onClick={() => onUpdateDuration(task.id, Math.max(5, task.duration - (task.duration >= 60 ? 30 : 5)))} 
                className="text-[9px] font-bold uppercase tracking-widest opacity-40 hover:opacity-100 transition-opacity"
              >
                -{task.duration >= 60 ? '30m' : '5m'}
              </button>
            ...
            <button 
                ...
                className="text-[9px] font-bold uppercase tracking-widest text-rose-500/60 hover:text-rose-500 transition-colors"
            >
                Remove
            </button>
          </div>
